...
def _normalize_nse_ticker(ticker: str) -> str:
    """Enforce NSE-only tickers. Accepts index symbols (^NSEI, ^NSEBANK) and
    auto-appends the Yahoo '.NS' suffix for equities (e.g. RELIANCE -> RELIANCE.NS).
    """
    t = ticker.strip().upper()
    nse_indices = {"^NSEI", "^NSEBANK", "^CNXIT", "^CNXAUTO", "^CNXFMCG"}
    if t in nse_indices:
        return t
    if t.startswith("^"):
        raise ValueError(
            f"'{ticker}' is not an NSE index. This skill is NSE-only. "
            f"Use one of {sorted(nse_indices)} or an equity like RELIANCE / TCS."
        )
    if t.endswith(".NS"):
        return t
    if "." in t:
        raise ValueError(
            f"'{ticker}' is not an NSE symbol. This skill is NSE-only - "
            "pass an NSE equity (e.g. RELIANCE, TCS, INFY) or index (^NSEI, ^NSEBANK)."
        )
    return t + ".NS"
...
def main() -> int:
    ...
    parser.add_argument("--ticker", default="^NSEI", help="NSE equity (e.g. RELIANCE) or index (^NSEI, ^NSEBANK)")
    ...
    ticker = _normalize_nse_ticker(args.ticker)
    print(f"\nmarkov-hedge-fund-method (NSE-only) - ticker={ticker} years={args.years} window={args.window}")
    print(f"  fetching {ticker} from Yahoo Finance...")
    df = _fetch_with_retry(ticker, args.years)
